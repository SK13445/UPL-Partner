...
const FranchiseDashboardHome = () => {
  ...
  const handleAcceptAgreement = async () => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to accept agreement"
      );
    }
  };
  ...
  const steps = [
    {
      ...
      component: (
        <ProfileForm franchise={franchise} onUpdate={fetchFranchiseData} />
      ),
    },
    {
      id: 2,
      ...
      status: agreementAccepted
        ? "complete"
        : franchise?.profileStatus === "complete"
        ? "pending"
        : "locked",
      component: (
        <AgreementSection
    ...
    <div>
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">
          Welcome, {franchise?.ownerName || "Partner"}!
        </h2>
        <p className="text-gray-600">
          Franchise Code:{" "}
          <span className="font-semibold">{franchise?.franchiseCode}</span>
        </p>
      </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg max-h-96 overflow-y-auto">
              ...
              <p className="mb-4 text-sm">
                The Franchise agrees to operate the business in accordance with
                the standards set by the Company.
              </p>

              <h4 className="font-semibold mb-2">2. Confidentiality</h4>
              <p className="mb-4 text-sm">
                The Franchise shall maintain confidentiality of all proprietary
                information.
              </p>

              <h4 className="font-semibold mb-2">3. Fees and Royalties</h4>
              <p className="mb-4 text-sm">
                The Franchise agrees to pay all applicable fees and royalties as
                per the schedule.
              </p>

              <h4 className="font-semibold mb-2">4. Termination</h4>
              <p className="mb-4 text-sm">
                The Company reserves the right to terminate this agreement in
                case of breach of terms.
              </p>

              <h4 className="font-semibold mb-2">5. Compliance</h4>
              <p className="mb-4 text-sm">
                The Franchise shall comply with all local, state, and federal
      ...
      {accepted ? (
        <div>
          <p className="text-green-600 mb-4">
            ...
            {new Date(
              agreementStatus?.agreementAcceptedAt
            ).toLocaleDateString()}
          </p>
          <button
            onClick={onPrint}
